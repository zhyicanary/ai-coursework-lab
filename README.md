...
START → orchestrator → parallel (asyncio.gather)
                        ├── 🌤️ WeatherAgent    (无依赖)
                        ├── ✈️ TransportAgent   (无依赖)
                        └── 🏨 HotelAgent       (无依赖)
                      → planning (顺序)
                        ├── 🗺️ ItineraryAgent  (依赖: 天气+交通)
...
| 模块 | 文件 | 作用 |
| --- | --- | --- |
| LLM 客户端 | `common/llm_client.py` | DeepSeek / Ollama 双后端热切换，运行时持久化到 `.env` |
| Embedding | `common/embedding_client.py` | Ollama 本地嵌入模型（qwen3-embedding:8b） |
...
│   ├── embedding_client.py        # Ollama 本地嵌入模型
...
│   │   ├── weather.py             # 🌤️ 天气 Agent
│   │   ├── transport.py           # ✈️ 交通 Agent
│   │   ├── hotel.py               # 🏨 住宿 Agent
│   │   ├── itinerary.py           # 🗺️ 行程 Agent
│   │   ├── budget.py              # 💰 预算 Agent
